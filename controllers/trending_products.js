...
exports.trending_product_search = (req, res, next) => {
...
exports.trending_create = (req, res, next) => {
    Trending.find({PRODUCT_ID:req.body.PRODUCT_ID})
...
exports.trending_update_by_id = (req, res, next) =>{
   ...
   updateOps['UPDATED_DATE'] = new Date();
    Trending.update({ _id: id }, { $set: updateOps },{multi: true})
        .exec()
        .then(result => {
            res.status(200).json({
                status: "success",

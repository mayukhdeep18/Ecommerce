...
exports.collection_product_search = (req, res, next) => {
...
//add in collection table
exports.collection_create = (req, res, next) => {
    var coll_id = req.body.COLLECTIONS_NAME.replace(/[^a-zA-Z0-9]/g,'-');
    Collections.find({PRODUCT_ID:req.body.PRODUCT_ID})
        ...
        .then(doc => {
            ...
            {
                const trending = new Collections({
                    _id: new mongoose.Types.ObjectId(),
                    COLLECTIONS_ID: coll_id.toLowerCase(),
                ...
                trending
                    ...
                    .then(result => {
                        res.status(201).json({
                            ...
                            data: {
                                message: "collections details stored"
                            }
                        });
                    })
                    .catch(err => {
...
exports.get_all_collection = (req, res, next) => {
    Collections.find()
        .select('COLLECTIONS_ID COLLECTIONS_NAME UPDATED_DATE UPDATED_BY ACTIVE_FLAG _id')
        .populate('PRODUCT_ID')
        .exec()
        .then(docs => {
            ...
            if (docs.length > 0) {
                const response = {
                    trending: docs.map(prod_item => {
                        return {
                            coll_doc_id: prod_item._id,
                            collection_id: prod_item.COLLECTIONS_ID,
                            collection_name: prod_item.COLLECTIONS_NAME,
...
exports.collection_update_by_id = (req, res, next) =>{
    const id = req.params.trendId;
    const updateOps = {};
    var coll_id = req.body.COLLECTIONS_NAME.replace(/[^a-zA-Z0-9]/g,'-');
    updateOps['COLLECTIONS_ID'] = coll_id.toLowerCase();
    updateOps['COLLECTIONS_NAME'] = req.body.COLLECTIONS_NAME;
    updateOps['PRODUCT_ID'] = req.body.PRODUCT_ID;
    updateOps['ACTIVE_FLAG'] = req.body.ACTIVE_FLAG;
    updateOps['UPDATED_DATE'] = new Date();
    Collections.update({ _id: id }, { $set: updateOps },{multi: true})
        .exec()
        .then(result => {
            res.status(200).json({
                status: "success",
